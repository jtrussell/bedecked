...
var rmUserAsset = function(type, name) {
  var assetPath = path.join(regDir, 'deck.js', 'themes', type, name + '.css');
  if(fs.existsSync(assetPath)) {
    fs.unlinkSync(assetPath);
  } else {
    console.log('Remove What? Path "' + assetPath + '" does not exist.');
  }
};
...
module.exports = function(program) {
  ...
  if(program.rmTheme || program.rmTransition) {
    ...
    if(themeName && themeName !== true) {
      rmUserAsset('style', themeName);
    }

    if(transName && transName !== true) {
